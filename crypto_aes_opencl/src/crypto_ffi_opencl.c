...
{
    const char* env = getenv("CRYPTO_OCL_CHUNK_BYTES");
    const char* env_compat = getenv("CRYPTO_CTR_CHUNK_BYTES");
    size_t v;
...
crypto_status_t crypto_ffi_opencl_warmup(void)
{
    uint8_t key[16];
    uint8_t iv[16];
    uint8_t in[16];
    ...
    return crypto_ocl_aes_ctr_xor(key, sizeof(key), iv, in, out, sizeof(in), 0, NULL);
}

void crypto_ffi_opencl_shutdown(void)
{
    crypto_ocl_shutdown();
}

const char* crypto_ffi_opencl_last_error_message(void)
{
                                                       ...
                                                       size_t* ciphertext_len_out)
{
    ...
    st = crypto_ocl_aes_ctr_xor(key, key_len_bytes, iv16, in_tmp, out_tmp, out_len, 0, NULL);
...
crypto_status_t crypto_ffi_opencl_aes_ctr_decrypt_alloc(const uint8_t* key,
                                                       size_t key_len_bytes,
                                                       const uint8_t iv16[16],
                                                       crypto_padding_t padding,
                                                       ...
                                                       size_t* plaintext_len_out)
{
    ...
    st = crypto_ocl_aes_ctr_xor(key, key_len_bytes, iv16, ciphertext, tmp, ciphertext_len, 0, NULL);
    if (st != CRYPTO_OK) {
...
static crypto_status_t file_encrypt_internal_round_keys(const uint8_t* round_keys,
                                                       ...
                                                       FILE* fout)
{
    ...
    while (1) {
        ...
        if (full_len) {
            st = crypto_ocl_aes_ctr_xor_round_keys(round_keys, round_keys_len, iv16, in_buf, out_buf, full_len, block_offset, NULL);
            if (st != CRYPTO_OK) {
                free(in_buf);
                free(out_buf);
                return st;
...

    {
        ...
        st = crypto_ocl_aes_ctr_xor_round_keys(round_keys, round_keys_len, iv16, final_block, final_out, 16, block_offset, NULL);
        if (st != CRYPTO_OK) {
...
crypto_status_t crypto_ffi_opencl_aes_ctr_encrypt_file(const uint8_t* key,
                                                      size_t key_len_bytes,
                                                      const uint8_t iv16[16],
                                                      crypto_padding_t padding,
                                                      ...
                                                      int prefix_iv)
{
    ...
    st = file_encrypt_internal_round_keys(aes.round_keys, crypto_aes_round_keys_bytes(&aes), iv16, padding, fin, fout);
...
static crypto_status_t file_decrypt_internal_round_keys(const uint8_t* round_keys,
                                                       size_t round_keys_len,
                                                       const uint8_t iv16[16],
                                                       crypto_padding_t padding,
                                                       FILE* fin,
                                                       FILE* fout)
{
    ...
    while (1) {
        ...
        if (process_len) {
            st = crypto_ocl_aes_ctr_xor_round_keys(round_keys, round_keys_len, iv16, buf, out_buf, process_len, block_offset, NULL);
            if (st != CRYPTO_OK) {
                free(buf);
                free(out_buf);
                return st;
...

    {
        ...
        st = crypto_ocl_aes_ctr_xor_round_keys(round_keys, round_keys_len, iv16, buf, last_plain, 16, block_offset, NULL);
        if (st != CRYPTO_OK) {
...
crypto_status_t crypto_ffi_opencl_aes_ctr_decrypt_file(const uint8_t* key,
                                                      size_t key_len_bytes,
                                                      const uint8_t iv16[16],
                                                      crypto_padding_t padding,
                                                      const char* input_path,
                                                      ...
                                                      int prefix_iv)
{
    ...
    st = file_decrypt_internal_round_keys(aes.round_keys, crypto_aes_round_keys_bytes(&aes), used_iv, padding, fin, fout);
...
crypto_status_t crypto_ffi_opencl_aes_gcm_encrypt_alloc(const uint8_t* key,
                                                       size_t key_len_bytes,
                                                       const uint8_t* iv,
                                                       size_t iv_len,
                                                       const uint8_t* aad,
                                                       ...
                                                       uint8_t tag16_out[16])
{
    ...
    st = crypto_ocl_aes_gcm_encrypt(key, key_len_bytes, iv, iv_len, aad, aad_len, plaintext, plaintext_len, out, tag16_out, NULL);
    if (st != CRYPTO_OK) {
...
crypto_status_t crypto_ffi_opencl_aes_gcm_decrypt_alloc(const uint8_t* key,
                                                       size_t key_len_bytes,
                                                       const uint8_t* iv,
                                                       size_t iv_len,
                                                       const uint8_t* aad,
                                                       ...
                                                       size_t* plaintext_len_out)
{
    ...
    st = crypto_ocl_aes_gcm_decrypt(key, key_len_bytes, iv, iv_len, aad, aad_len, ciphertext, ciphertext_len, tag16, out, NULL);
    if (st != CRYPTO_OK) {
...
typedef struct opencl_gcm_xor_context_t {
    const uint8_t* round_keys;
    size_t round_keys_len;
    const uint8_t* j0;
} opencl_gcm_xor_context_t;

static crypto_status_t opencl_gcm_xor_callback(void* context,
                                              const uint8_t* input,
                                              uint8_t* output,
                                              size_t len,
                                              uint64_t block_offset)
{
    const opencl_gcm_xor_context_t* ctx = (const opencl_gcm_xor_context_t*)context;

    if (!ctx || !ctx->round_keys || !ctx->j0) {
        return CRYPTO_INVALID_ARG;
    }

    return crypto_ocl_aes_gcm_ctr_xor_round_keys(ctx->round_keys, ctx->round_keys_len, ctx->j0, input, output, len, block_offset, NULL);
}

crypto_status_t crypto_ffi_opencl_aes_gcm_encrypt_file(const uint8_t* key,
                                                      size_t key_len_bytes,
                                                      const uint8_t* iv,
                                                      size_t iv_len,
                                                      const uint8_t* aad,
                                                      ...
                                                      uint8_t tag16_out[16])
{
    ...
    opencl_gcm_xor_context_t xor_ctx;
    crypto_status_t st;
    size_t chunk;
    uint8_t* in_buf;
    uint8_t* out_buf;
    ...
    xor_ctx.round_keys = aes.round_keys;
    xor_ctx.round_keys_len = crypto_aes_round_keys_bytes(&aes);
    xor_ctx.j0 = gcm.j0;
    ...
    while (1) {
        ...
        st = crypto_ctr_stream_xor(&ctr, opencl_gcm_xor_callback, &xor_ctx, in_buf, out_buf, n);
        if (st != CRYPTO_OK) {
            break;
        }
...
crypto_status_t crypto_ffi_opencl_aes_gcm_decrypt_file(const uint8_t* key,
                                                      size_t key_len_bytes,
                                                      const uint8_t* iv,
                                                      size_t iv_len,
                                                      const uint8_t* aad,
...
void crypto_ffi_opencl_free(void* p)
{
    free(p);
}
